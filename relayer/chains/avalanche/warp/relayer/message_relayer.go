...
import (
	...
	warp2 "github.com/cosmos/relayer/v2/relayer/chains/avalanche/warp"
)

type blsSignatureBuf [bls.SignatureLen]byte

type AnyBinaryData struct {
	data []byte
}
...
func (r *messageRelayer) createSignedMessageAppRequest(requestID uint32) (*warp.Message, error) {
	...
	outMsg, err := r.messageCreator.AppRequest(r.warpMessage.SourceChainID, requestID, warp2.DefaultAppRequestTimeout, reqBytes)
	if err != nil {
		r.logger.Error(
...
func (r *messageRelayer) getCurrentCanonicalValidatorSet() ([]*warp.Validator, uint64, error) {
	...
	// The source subnet signs the message.
