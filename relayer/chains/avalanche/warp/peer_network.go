...
type AppRequestNetwork struct {
	...
	log        *zap.Logger // TODO keep only only logger
	lock       *sync.Mutex
}
...
func NewNetwork(
	...
	subnetIDs []ids.ID,
	APINodeURL string,
) (*AppRequestNetwork, chan message.InboundMessage, error) {
	...
	// Construct a response chan. Inbound messages will be routed to the proper channel in the handler
	responseChan := make(chan message.InboundMessage, InboundMessageChannelSize)
	responseChansLock := new(sync.RWMutex)

	handler, err := NewRelayerExternalHandler(logger, registerer, responseChan, responseChansLock)
	if err != nil {
		logger.Error(
